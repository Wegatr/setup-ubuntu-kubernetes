...
| `tekton` | 3 | Tekton Pipelines + Triggers operator install + (DEV-only) Tekton Dashboard UI at `tekton.dev.<DOMAIN_SUFFIX>`. Dashboard gated by `.Values.dashboard.enabled`, protected by the platform IdP via `idp-forwardauth@kubernetescrd`. See `apps/tekton/README.md`. |
